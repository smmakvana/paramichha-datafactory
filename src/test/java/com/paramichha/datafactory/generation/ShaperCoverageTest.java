...
class ShaperCoverageTest {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // StringShaper — additional semantic hints not covered above
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("StringShaper.shape() — additional hint branches")
    class StringShaperAdditionalHints {

        @Test
        void link_hint() {
            assertThat((String) StringShaper.shape(fieldOf("link", String.class), semantic())).isNotBlank();
        }

        @Test
        void website_hint() {
            assertThat((String) StringShaper.shape(fieldOf("website", String.class), semantic())).isNotBlank();
        }

        @Test
        void postal_hint() {
            assertThat((String) StringShaper.shape(fieldOf("postalCode", String.class), semantic())).isNotBlank();
        }

        @Test
        void ccy_hint() {
            assertThat((String) StringShaper.shape(fieldOf("ccy", String.class), semantic())).isNotBlank();
        }

        @Test
        void note_hint() {
            assertThat((String) StringShaper.shape(fieldOf("note", String.class), semantic())).isNotBlank();
        }

        @Test
        void comment_hint() {
            assertThat((String) StringShaper.shape(fieldOf("comment", String.class), semantic())).isNotBlank();
        }

        @Test
        void organisation_hint() {
            assertThat((String) StringShaper.shape(fieldOf("organisation", String.class), semantic())).isNotBlank();
        }

        @Test
        void ref_hint() {
            assertThat((String) StringShaper.shape(fieldOf("ref", String.class), semantic())).isNotBlank();
        }

        @Test
        void reason_hint() {
            assertThat((String) StringShaper.shape(fieldOf("reason", String.class), semantic())).isNotBlank();
        }
    }

    // ═══════════════════════════════════════════════════════════════════════
    // TemporalShaper — PastOrPresent / FutureOrPresent for Date, Year, YearMonth
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("TemporalShaper — PastOrPresent / FutureOrPresent for Date/Year/YearMonth")
    class TemporalPastOrPresentFutureOrPresent {

        @Test
        void date_pastOrNow() {
            var f = field("dt", java.util.Date.class, "@PastOrPresent");
            assertThat(((java.util.Date) TemporalShaper.shape(f, semantic()))
                    .before(new java.util.Date(System.currentTimeMillis() + 1000))).isTrue();
        }

        @Test
        void year_pastOrNow() {
            var f = field("y", Year.class, "@PastOrPresent");
            assertThat((Year) TemporalShaper.shape(f, semantic())).isLessThanOrEqualTo(Year.now());
        }

        @Test
        void yearMonth_pastOrNow() {
            var f = field("ym", YearMonth.class, "@PastOrPresent");
            assertThat((YearMonth) TemporalShaper.shape(f, semantic())).isLessThanOrEqualTo(YearMonth.now());
        }

        @Test
        void date_futureOrNow() {
            var f = field("dt", java.util.Date.class, "@FutureOrPresent");
            assertThat(((java.util.Date) TemporalShaper.shape(f, semantic()))
                    .after(new java.util.Date(System.currentTimeMillis() - 1000))).isTrue();
        }

        @Test
        void year_futureOrNow() {
            var f = field("y", Year.class, "@FutureOrPresent");
            assertThat((Year) TemporalShaper.shape(f, semantic())).isGreaterThanOrEqualTo(Year.now());
        }

        @Test
        void yearMonth_futureOrNow() {
            var f = field("ym", YearMonth.class, "@FutureOrPresent");
            assertThat((YearMonth) TemporalShaper.shape(f, semantic())).isGreaterThanOrEqualTo(YearMonth.now());
        }

        @Test
        void unknown_type_fallsBackToInstant() {
            // String field routed to TemporalShaper (non-temporal type) hits final Instant.now() fallback
            var f = AnnotationAnalyzer.analyzeFromStrings("f", "String", List.of("@Past"));
            assertThat(TemporalShaper.shape(f, semantic())).isInstanceOf(java.time.Instant.class);
        }
    }

    // ═══════════════════════════════════════════════════════════════════════
    // NumericShaper — additional branches
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("NumericShaper — double with fixed target")
    class NumericAdditional {

        @Test
        void double_atMin_target() {
            var f = AnnotationAnalyzer.analyzeFromStrings("n", "Double", List.of("@Min(1)", "@Max(10)"));
            double v = (double) NumericShaper.shape(f, atMin(1));
            assertThat(v).isEqualTo(1.0d);
        }

        @Test
        void bigDecimal_withDigits_atMin() {
            var f = AnnotationAnalyzer.analyzeFromStrings("price", "BigDecimal",
                    List.of("@Digits(integer=5, fraction=2)"));
            var v = (java.math.BigDecimal) NumericShaper.shape(f, atMin(5));
            assertThat(v.scale()).isEqualTo(2);
        }
    }
