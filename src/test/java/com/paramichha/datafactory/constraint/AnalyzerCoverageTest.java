...
class AnalyzerCoverageTest {
    ...
    class QuantityBoundsCoverage {
        ...
        void isContradictory_true() {
            assertThat(new QuantityBounds.IntegerBounds(10L, 5L).isContradictory()).isTrue();
        }

        @Test
        void isContradictory_false() {
            assertThat(new QuantityBounds.IntegerBounds(5L, 10L).isContradictory()).isFalse();
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // RuntimeAnnotationClassifier — additional format / bounds gaps
    // (merged from RuntimeClassifierGapTest + StringClassifierGapTest)
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("RuntimeAnnotationClassifier — additional gaps")
    class RuntimeGaps {

        @Test
        void uuid_string_format() throws Exception {
            var ann = (org.hibernate.validator.constraints.UUID)
                    UuidHolder.class.getDeclaredField("f").getAnnotations()[0];
            var fc = AnnotationAnalyzer.analyze("uid", String.class, java.util.List.of(ann));
            assertThat(fc.format()).isEqualTo(FormatType.UUID_STRING);
        }

        @Test
        void decimalMax_sets_upper_bound() throws Exception {
            var ann = (jakarta.validation.constraints.DecimalMax)
                    DecimalMaxHolder.class.getDeclaredField("f").getAnnotations()[0];
            var fc = AnnotationAnalyzer.analyze("price", java.math.BigDecimal.class, java.util.List.of(ann));
            assertThat(fc.bounds() instanceof QuantityBounds.DecimalBounds).isTrue();
            assertThat(((QuantityBounds.DecimalBounds) fc.bounds()).decMax()).isEqualByComparingTo("999");
        }

        @Test
        void length_zero_min_omitted() throws Exception {
            var ann = (org.hibernate.validator.constraints.Length)
                    LengthZeroMinHolder.class.getDeclaredField("f").getAnnotations()[0];
            var fc = AnnotationAnalyzer.analyze("name", String.class, java.util.List.of(ann));
        ...
        void length_maxint_max_omitted() throws Exception {
            var ann = (org.hibernate.validator.constraints.Length)
                    LengthMaxIntHolder.class.getDeclaredField("f").getAnnotations()[0];
            var fc = AnnotationAnalyzer.analyze("name", String.class, java.util.List.of(ann));
        ...
        void char_primitive_resolves_to_Character() {
            var fc = AnnotationAnalyzer.analyzeFromStrings("c", "char", java.util.List.of());
            assertThat(fc.fieldType()).isEqualTo(Character.class);
        }

        @Test
        void Character_wrapper_resolves_to_Character() {
            var fc = AnnotationAnalyzer.analyzeFromStrings("c", "Character", java.util.List.of());
            assertThat(fc.fieldType()).isEqualTo(Character.class);
        }

        // annotation holder inner classes
        static class UuidHolder {
            @org.hibernate.validator.constraints.UUID
            String f;
        }
        static class DecimalMaxHolder {
            @jakarta.validation.constraints.DecimalMax("999")
        ...
        static class LengthMaxIntHolder {
            @org.hibernate.validator.constraints.Length(min = 3, max = Integer.MAX_VALUE)
            String f;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // StringAnnotationClassifier — branch gaps
    // (merged from StringClassifierGapTest)
    // ═══════════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("StringAnnotationClassifier — branch gaps")
    class StringClassifierGaps {

        private final StringAnnotationClassifier C = StringAnnotationClassifier.INSTANCE;

        @Test
        void classifyFormat_isbn()  { assertThat(C.classifyFormat("@ISBN")).isEqualTo(FormatType.ISBN); }
        @Test
        void classifyFormat_ean()   { assertThat(C.classifyFormat("@EAN")).isEqualTo(FormatType.EAN); }
        @Test
        void classifyFormat_none()  { assertThat(C.classifyFormat("@NotNull")).isEqualTo(FormatType.NONE); }

        @Test
        void extractPatternRegexp_no_regexp_keyword_returns_null() {
            assertThat(C.extractPatternRegexp("@NotBlank")).isNull();
        }
        @Test
        void extractPatternRegexp_no_opening_quote_returns_null() {
            assertThat(C.extractPatternRegexp("@Pattern(regexp=noQuote)")).isNull();
        }
        @Test
        void extractPatternRegexp_no_closing_quote_returns_null() {
            assertThat(C.extractPatternRegexp("@Pattern(regexp=\"noclose")).isNull();
        }
        @Test
        void extractPatternRegexp_valid() {
            assertThat(C.extractPatternRegexp("@Pattern(regexp=\"^[A-Z]+$\")")).isEqualTo("^[A-Z]+$");
        }

        @Test
        void classifyQuantity_min_no_digits_returns_null()  { assertThat(C.classifyQuantity("@Min(abc)")).isNull(); }
        @Test
        void classifyQuantity_max_no_digits_returns_null()  { assertThat(C.classifyQuantity("@Max(abc)")).isNull(); }
        @Test
        void classifyQuantity_size_both_trimmed_returns_null() {
            assertThat(C.classifyQuantity("@Size(min=0, max=2147483647)")).isNull();
        }
        @Test
        void classifyQuantity_length_both_trimmed_returns_null() {
            assertThat(C.classifyQuantity("@Length(min=0, max=2147483647)")).isNull();
        }
        @Test
        void classifyQuantity_min_returns_bounds() {
            var b = C.classifyQuantity("@Min(18)");
            assertThat(b).isNotNull();
            assertThat(b.min()).isEqualTo(18L);
        }
        @Test
        void classifyQuantity_max_returns_bounds() {
        ...
        @Test
        void extractLong_negative_value_parsed() {
            var b = C.classifyQuantity("@Min(-5)");
            assertThat(b).isNotNull();
            assertThat(b.min()).isEqualTo(-5L);
        }
        @Test
        void extractLong_negative_max_parsed() {
            var b = C.classifyQuantity("@Max(-1)");
            assertThat(b).isNotNull();
            assertThat(b.max()).isEqualTo(-1L);
        }
        @Test
        void extractLong_empty_returns_null() {
            assertThat(C.classifyQuantity("@Min()")).isNull();
        }

        @Test
        void classifyTemporal_none()    { assertThat(C.classifyTemporal("@NotNull")).isEqualTo(TemporalDirection.NONE); }
        @Test
        void classifyTemporal_past()    { assertThat(C.classifyTemporal("@Past")).isEqualTo(TemporalDirection.PAST); }
        @Test
        void classifyTemporal_future()  { assertThat(C.classifyTemporal("@Future")).isEqualTo(TemporalDirection.FUTURE); }
    }
}
